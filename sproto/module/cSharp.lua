...
local function dump_class(class_info, stream, deep)
  ...
  if sproto_type then
    ...
    for i=1,#sproto_type do
      ...
      if (name == "received_mark") then
        --print(type .. " " .. name)
      end

      if (class_name == name) then
        print("renamed, class same as name " .. name)    
        field.name = field.name .. "_"
      end

      if (name == "public" or name == "base" or name == "switch" or name == "params") then
        print("renamed, c# keywords" .. name)
        field.name = field.name .. "_"
      end

      if (ends_with(type, name) or ends_with(type, name .. '>')) then
        --print("renamed, type same as name" .. " " .. type .. ": " .. name)
        field.name = field.name .. "_"
      end
    end
