...
local function parser(text, filename, namespace, build)
	...
	for line in string.gmatch(text, ".-\n}") do		
		local f = lpeg.match(proto * -1 + exception , line , 1, state)
		table.insert(r, f[1])
	end
...
local function fix_request(ast)
	local map = {}
	local type = ast.type
	for protocol_name, v in pairs(ast.protocol) do
		local tag = v.tag
		local request = v.request
		local response = v.response
		local p = map[tag]	

		if response and not request then
			 v.request = protocol_name..".request"
			 type[v.request] = {
				order = 1,
				file = "wof/proto.sproto",
				line = 412
			 }
		end
	end
